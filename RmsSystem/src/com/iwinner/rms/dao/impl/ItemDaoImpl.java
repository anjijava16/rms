...
public class ItemDaoImpl implements ItemDaoIF {
	...
	public Integer updateItem(ItemInfo itemInfo) throws DaoException {
		...
		session.beginTransaction().begin();
		try {
			ItemInfo item = (ItemInfo) session.load(ItemInfo.class,itemInfo.getItemId());
			if (item != null) {
				System.out.println("######################");
				System.out.println(item.getItemId()+"  "+item);
				System.out.println("######################");
				session.merge(itemInfo);
				session.beginTransaction().commit();
...
public List<Integer> getItemIds()throws DaoException{
	...
	Session session = HibernateUtils.getSession();
	Query query=session.createQuery(RMSConstants.SELECT_ITEMS_IDS);
	listOfIds=(List<Integer>)query.list();
	return listOfIds;
}
